...
local user_opts = {
    ...
    boxalpha = 150, -- alpha of the background box,
    -- 0 (opaque) to 255 (fully transparent)
    hidetimeout = 30000, -- duration in ms until the OSC hides if no
...
local osc_styles = {
    ...
    smallButtonsLlabel = "{\\fscx105\\fscy105\\fn" ..
    ...
    smallButtonsBarS = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs18\\fnmpv-osd-symbols}",
    smallButtonsBarF = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs26\\fnmpv-osd-symbols}",
    timecodesBar = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs22}",
    timecodesBack = "{\\blur0\\bord0\\1c&H979797\\3c&HFFFFFF\\fs18\\q2}",
    timePosBar = "{\\blur0\\bord" .. user_opts.tooltipborder ..
        "\\1c&HFFFFFF\\3c&H000000\\fs22}",
...
function set_track(type, next)
    ...
    if (new_track_osc == 0) then
        ...
        show_message(nicetypes[type] .. " Track: " .. new_track_osc .. "/" ..
                         #tracks_osc[type] .. " [" ..
    ...
    for _, element in pairs(elements) do
        ...
        elseif (element.type == "slider") then
            ...
            ass_draw_rr_h_ccw(static_ass, slider_lo.border, slider_lo.border,
                              ...
                              elem_geo.h - slider_lo.border, r2,
                              slider_lo.stype == "diamond")
                    ...
                    if (marker > element.slider.min.value) and
                        ...
                        if (slider_lo.gap > 1) then -- draw triangles
                            ...
                            if (slider_lo.nibbles_bottom) then
                                static_ass:move_to(s - (a / 2), elem_geo.h -
                                                       slider_lo.border)
                                static_ass:line_to(s, elem_geo.h - foV)
                                static_ass:line_to(s + (a / 2), elem_geo.h -
                                                       slider_lo.border)
                            end
                            ...
                            -- bottom
                            if (slider_lo.nibbles_bottom) then
                                static_ass:rect_cw(s - 1, elem_geo.h -
                                                       slider_lo.border -
                                                       slider_lo.gap, s + 1,
        ...
        if (element.type == "slider") then
            ...
            if pos then
                ...
                if slider_lo.stype ~= "bar" then
                    ...
                    ass_draw_rr_h_cw(elem_ass, xp - r, foH - r, xp + r, foH + r,
                    ...
                    if seekRanges and not user_opts.seekrangeseparate and
                        ...
                        for _, range in pairs(seekRanges) do
                            if range["start"] < pos then
                                local pstart =
                                    get_slider_ele_pos_for(element,
                                                           range["start"])
                                local pend = xp
                                ...
                                elem_ass:rect_ccw(pstart, elem_geo.h - foV -
                                                      seekRangeLineHeight, pend,
                                                  elem_geo.h - foV)
                            end
                        end
                    end
            ...
            if seekRanges then
                ...
                for _, range in pairs(seekRanges) do
                    ...
                    elseif slider_lo.rtype == "line" then
                        if slider_lo.stype == "bar" then
                            elem_ass:rect_cw(pstart, elem_geo.h - foV -
                                                 seekRangeLineHeight, pend,
                                             elem_geo.h - foV)
                        else
                            ass_draw_rr_h_cw(elem_ass, pstart - innerH / 8,
                                             foH - innerH / 8,
                                             pend + innerH / 8,
                                             foH + innerH / 8, innerH / 8,
                                             slider_lo.stype == "diamond")
                        end
                    ...
                    elseif slider_lo.rtype == "inverted" then
                        if slider_lo.stype ~= "bar" then
                            ass_draw_rr_h_ccw(elem_ass, pstart,
                                              (elem_geo.h / 2) - 1, pend,
                                              (elem_geo.h / 2) + 1, 1,
                                              slider_lo.stype == "diamond")
                        else
                            elem_ass:rect_ccw(pstart, (elem_geo.h / 2) - 1,
                                              pend, (elem_geo.h / 2) + 1)
                        end
                    end
                end
...
function get_playlist()
    ...
    if count == 0 then return 'Empty playlist.' end
...
function get_chapterlist()
    ...
    if count == 0 then return 'No chapters.' end

    local message = string.format('Chapters [%d/%d]:\n', pos, count)
    for i, v in ipairs(limlist) do
        local time = mp.format_time(v.time)
        local title = v.title
        if title == nil then title = string.format('Chapter %02d', i) end
...
function render_message(ass)
    if not (state.message_timeout == nil) and not (state.message_text == nil) and
        state.message_timeout > mp.get_time() then
...
layouts["box"] = function()
    ...
    if (osc_param.playresx < (osc_geo.w + (2 * osc_geo.p))) then
        osc_param.playresy = (osc_geo.w + (2 * osc_geo.p)) /
                                 osc_param.display_aspect
    ...
    if user_opts.valign > 0 then
        ...
        sh_area_y1 = (posY + (osc_geo.h / 2)) +
                         get_align(1 - (2 * user_opts.deadzonesize),
                                   osc_param.playresy - (posY + (osc_geo.h / 2)),
                                   0, 0)
    end
    add_area("showhide", 0, sh_area_y0, osc_param.playresx, sh_area_y1)

    -- fetch values
    local osc_w, osc_h, osc_r, osc_p = osc_geo.w, osc_geo.h, osc_geo.r,
                                       osc_geo.p
...
layouts["bottombar"] = function()
    ...
    local padY = 3
    local buttonW = 27
    local tcW = (state.tc_ms) and 118 or 80 -- 170 or 110
    local tsW = 40 -- 90
    ...
    geo = {
        ...
        w = tsW,
        h = geo.h
    } -- { x = geo.x - tsW - padX, y = geo.y, an = geo.an, w = tsW, h = geo.h }
    lo = add_layout("cy_sub")
    lo.geometry = geo
    lo.style = osc_styles.smallButtonsBarS
    ...
    -- Cache
    geo = {x = geo.x - geo.w - padX, y = geo.y, an = geo.an, w = 50, h = geo.h}
    lo = add_layout("cache")
    lo.geometry = geo
    lo.style = osc_styles.vidtitleBar

    local t_r = geo.x - geo.w - padX

    -- Title
    geo = {x = t_l, y = geo.y, an = 4, w = t_r - t_l, h = geo.h}
    lo = add_layout("title")
    lo.geometry = geo
    lo.style = string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.vidtitleBar,
    ...
    -- lo = add_layout("tog_fs")
    -- lo.geometry = geo
    -- lo.style = osc_styles.smallButtonsBarF
    ...
    lo.layer = 15
    lo.style = osc_styles.timecodesBar
    lo.alpha[1] = math.min(255,
                           user_opts.boxalpha + (255 - user_opts.boxalpha) * 0.2)
    if not (user_opts["seekbarstyle"] == "bar") then
...
layouts["topbar"] = function()
    ...
    -- Fullscreen button
    geo = {
        x = osc_geo.x + osc_geo.w - buttonW - padX,
        y = geo.y,
        an = 4,
        w = buttonW,
        h = geo.h
    ...
    lo.style = osc_styles.timecodesBar
    lo.alpha[1] = math.min(255,
                           user_opts.boxalpha + (255 - user_opts.boxalpha) * 0.8)
    if not (user_opts["seekbarstyle"] == "bar") then
        lo.box.radius = geo.h / 2
        lo.box.hexagon = user_opts["seekbarstyle"] == "diamond"
    end
...
function validate_user_opts()
    ...
    if user_opts.seekbarstyle ~= "bar" and user_opts.seekbarstyle ~= "diamond" and
    ...
    if user_opts.seekrangestyle ~= "bar" and user_opts.seekrangestyle ~= "line" and
        user_opts.seekrangestyle ~= "slider" and user_opts.seekrangestyle ~=
        "inverted" and user_opts.seekrangestyle ~= "none" then
...
function osc_init()
    ...
    ne.eventresponder["mbtn_left_up"] = function()
        ...
        if (have_pl) then
            title = string.format("[%d/%d] %s", countone(pl_pos - 1), pl_count,
                                  title)
        end
        show_message(title)
    end
    ...
    ne.content = function()
        local aid = "–"
        if not (get_track("audio") == 0) then aid = get_track("audio") end
        return ("\238\132\134" .. osc_styles.smallButtonsLlabel .. " " .. aid ..
                   "/" .. #tracks_osc.audio)
    end
    ...
    ne.content = function()
        local sid = "–"
        if not (get_track("sub") == 0) then sid = get_track("sub") end
        return ("\238\132\135" .. osc_styles.smallButtonsLlabel .. " " .. sid ..
                   "/" .. #tracks_osc.sub)
    end
    ...
    ne.slider.seekRangesF = function()
        if user_opts.seekrangestyle == "none" then return nil end
    ...
    ne.content = function()
        ...
        if dmx_cache and
            (dmx_cache > state.dmx_cache * 1.1 or dmx_cache < state.dmx_cache *
                0.9) then
        ...
        return "Cache: " ..
                   (min > 0 and string.format("%sm%02.0fs", min, sec) or
...
function mouse_leave()
    if user_opts.hidetimeout >= 0 then hide_osc() end
    -- reset mouse position
    state.last_mouseX, state.last_mouseY = nil, nil
end

function request_init() state.initREQ = true end
...
function render()
    ...
    -- autohide
    if not (state.showtime == nil) and (user_opts.hidetimeout >= 0) and
        (state.showtime + (user_opts.hidetimeout / 1000) < now) and
        (state.active_element == nil) and not (mouse_over_osc) then
...
mp.set_key_bindings({
    {
        ...
        "shift+mbtn_left",
        function(e) process_event("shift+mbtn_left", "up") end,
    ...
    {"mbtn_left_dbl", "ignore"}, {"shift+mbtn_left_dbl", "ignore"},
